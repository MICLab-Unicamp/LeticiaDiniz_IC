...
config_path = input("Enter path for YAML file with hop study description: ")
...
name_of_study = str(config.get('name_of_study','hop_study'))
...
if param_to_vary != 'hop':
    raise Exception('This code only handles hop variation studies.')
try:
...
hop_ = np.arange(min_param,max_param,step_param).astype('int')
...
if win == 'hann':
    ...
    window_ = boxcar(mfft_,sym=True)
elif win == 'flat':
    window_ = flattop(mfft_,sym=True)
else:
...
if norm_ != 'abs' and norm_ != 'm1p1' and norm_ != 'minmax' and norm_ != 'zscore':
    raise Exception('Unknown normalization. Check function get_normalized_spectrogram in utils.py and addapt it to handle the desired normalization. Check also FWHM and Statistics calculation if norm is not zero centered.') 
...
if add_noise_to_fids == True:
    print('Running hop study on '+str(qntty)+' noisy transients.')
else:
    print('Running hop study on '+str(qntty)+' GT transients.')
    
print('Hop variation: ['+str(min_param)+','+str(max_param)+') stepping '+
...
##--------------------GENERATING GABA SPECTROGRAMS FROM FIDS FOR EVERY HOP VALUE------
spgram_hop = {}
print('Creating GABA spectrograms for every hop value and every fid signal. \nThis might take a while...')
for i in range(hop_.shape[0]):
    if add_noise_to_fids == True:
        spgram, freq_spect, ppm_spect, t_spect = utils.get_normalized_spectrogram(fids=np.mean(corrupted_fids[:,:,1,:]-corrupted_fids[:,:,0,:],axis=2),bandwidth=bandwidth,window=window_,mfft=mfft_,hop=int(hop_[i]),norm=norm_,correct_time=True,a=a,b=b)
    else:
        spgram, freq_spect, ppm_spect, t_spect = utils.get_normalized_spectrogram(fids=gt_fids[:,:,1]-gt_fids[:,:,0],bandwidth=bandwidth,window=window_,mfft=mfft_,hop=int(hop_[i]),norm=norm_,correct_time=True,a=a,b=b)
    spgram_hop['hop_'+str(hop_[i])] = [spgram, freq_spect, ppm_spect, t_spect]
...
for i in range(len(hop_)):
    if norm_ == 'minmax':
        aux_mean_minmax = np.mean(np.real(spgram_hop['hop_'+str(hop_[i])][0]),axis=(1,2),keepdims=True)
        aux_minmax = np.real(spgram_hop['hop_'+str(hop_[i])][0]) - aux_mean_minmax
...
if save_pictures_along_the_way == True:
    ...
    aux_concat = (np.arange(0,len(hop_),int(len(hop_)/6))).tolist()
    ...
    im = ax[0].imshow(np.real(spgram_hop_concat[idx_freq_1ppm[0]:idx_freq_4ppm[0],:]), origin='lower', aspect='auto',cmap='gray',vmin=-0.04,vmax=0.04,
    ...
    im = ax[1].imshow(np.abs(np.real(spgram_hop_concat[idx_freq_1ppm[0]:idx_freq_4ppm[0],:])), origin='lower', aspect='auto',cmap='gray',vmin=0,vmax=0.04,
            extent = (0,spgram_hop_concat.shape[-1],np.flip(spgram_hop['hop_'+str(hop_[0])][2])[idx_freq_1ppm[0]],np.flip(spgram_hop['hop_'+str(hop_[0])][2])[idx_freq_4ppm[0]]))
    fig.colorbar(im, ax = ax[1])
    ax[0].set_title('Concat Spectrograms with hop $\in$ ['+str(hop_[0])+','+str(hop_[-1])+') \n with step '+str(int(len(hop_)/6)))
    ax[1].set_title('Concat |Spectrograms| with hop $\in$ ['+str(hop_[0])+','+str(hop_[-1])+') \n with step '+str(int(len(hop_)/6)))
    ax[0].set_ylabel('Chemical Shift [ppm]')
    ax[1].set_ylabel('Chemical Shift [ppm]')
    ...
    plot_id = np.arange(0,len(hop_),int(len(hop_)/6))
    ...
    ##--------------------FWHM MEASURED IN ABS PROJECTION------------------------------
    ...
    ax[0].set_title('FWHM in Proj(|Spectrogram|) x hop')
    ax[0].set_xlabel('hop')
    ...
    ax[1].set_title('FWHM in Proj(|Spectrogram|) x hop')
    ax[1].set_xlabel('hop')
    ax[1].set_ylabel('FWHM in Proj(|Spectrogram|)')
    ax[1].legend(loc='upper left')
    plt.tight_layout()
    plt.savefig(results_folder+'fwhm_abs_proj.png')
    plt.close()

    ##--------------------FWHM MEASURED IN REAL PROJECTION-----------------------------
    fig,ax = plt.subplots(1,2,figsize=(15,3))
    ...
    ax[1].legend(loc='upper left')
    plt.tight_layout()
    plt.savefig(results_folder+'fwhm_real_proj.png')
    plt.close()
    ...
    ax1.set_title('ZCR for peak lines x hop')
    ax1.set_ylabel('ZCR')
    ax1.set_xlabel('hop')

    ax2.imshow(np.real(spgram_hop_concat[idx_freq_1ppm[0]:idx_freq_4ppm[0],:]), origin='lower', aspect='auto',cmap='gray',vmin=-0.04,vmax=0.04,
            extent = (0,spgram_hop_concat.shape[-1],np.flip(spgram_hop['hop_'+str(hop_[0])][2])[idx_freq_1ppm[0]],np.flip(spgram_hop['hop_'+str(hop_[0])][2])[idx_freq_4ppm[0]]))
    ax2.set_xlabel('Chemical Shift [ppm]')
    ax2.set_ylabel('Pixels')
    ax2.set_title('Concat Spectrograms with hop $\in$ ['+str(hop_[0])+','+str(hop_[-1])+') \n with step '+str(int(len(hop_)/6)))

    for i in range(0,len(hop_),int(len(hop_)/6)):
    ...
    ax3.set_title('NAA Peak till 0.4s center in 0')
    ax3.set_ylabel('NAA Profile')
    ax3.set_xlabel('Columns')
    ax4.set_title('GABA Peak till 0.4s center in 0')
    ax4.set_ylabel('GABA Profile')
    ax4.set_xlabel('Columns')
    ax5.set_title('Glx Peak till 0.4s center in 0')
    ax5.set_ylabel('Glx Profile')
...
if perform_stats_analysis == True:
    ...
    if save_pictures_stats == True:
        ...
        for i in range(len(plot_seq)):
            for j in range(len(plot_seq[i])):
                ...
                ax.flat[i].plot(hop_,aux,label =str(sections[plot_seq[i][j]])+':'+str(sections[plot_seq[i][j]+1]),marker='o',color=colors[j])
                ax.flat[i].fill_between(hop_, np.array(aux) - np.array(aux_std), 
        ...
        for i in range(5):
            ...
            ax.flat[i].set_title('PPM Regions Max Abs Values x hop')
    ...
    ##--------------------QNTTY OF PIXELS PER SEGMENTED REGION x HOP--------------
    if norm_ == 'minmax':
        hist, bins, bins_centered, qntty_percent_regions = funcstud.histogram_for_different_spgram_and_qntty_per_histogram_region(spgram_dict=spgram_hop,nbins=5000,part='real',regions=segmentation_values,normalized=True,not_zero_centered=True,center_value=0.5)
        hist_absolute, bins_absolute, bins_centered_absolute, qntty_absolute_regions = funcstud.histogram_for_different_spgram_and_qntty_per_histogram_region(spgram_dict=spgram_hop,nbins=5000,part='real',regions=segmentation_values,normalized=False,not_zero_centered=True,center_value=0.5)
    else:
        hist, bins, bins_centered, qntty_percent_regions = funcstud.histogram_for_different_spgram_and_qntty_per_histogram_region(spgram_dict=spgram_hop,nbins=5000,part='real',regions=segmentation_values,normalized=True)
        hist_absolute, bins_absolute, bins_centered_absolute, qntty_absolute_regions = funcstud.histogram_for_different_spgram_and_qntty_per_histogram_region(spgram_dict=spgram_hop,nbins=5000,part='real',regions=segmentation_values,normalized=False)
    ...
    if save_pictures_stats == True:
        ...
        for j in range(len(stat_name)):
            for i in range(len(list(stats_per_region[stat_name[j]].keys()))):
                ...
                ax[i,j].fill_between(hop_, aux - aux_std, 
                                aux + aux_std, alpha=0.35, color = colors[i])
                if i == 0:
                    ax[i,j].set_title(stat_name[j]+' x hop')
                if j == 0:
        ...
        for i in range(len(stat_name)):
            ...
            ax[nrows-1,i].plot(hop_,aux,marker='o',color=colors[0],label="global")
            ax[nrows-1,i].fill_between(hop_, aux - aux_std, 
                            aux + aux_std, alpha=0.35, color = colors[0])
            if i == 0:
                ax[nrows-1,i].set_ylabel('Global Spgram')
        for i in range(5):
            ax[nrows-1,i].set_xlabel('hop')
        plt.tight_layout()
